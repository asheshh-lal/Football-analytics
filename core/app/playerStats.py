...
def upload_csv(request):
    if request.method == 'POST':
        if 'csv_file' in request.FILES:
            ...
            unique_players = pd.DataFrame(csv_data)[['player', 'team']].drop_duplicates()
            player_choices = [(row['player'], f"{row['player']} ({row['team']})") for _, row in unique_players.iterrows()]
            ...
            if choice_form.is_valid():
                ...
                data = pd.DataFrame(request.session.get('csv_data'))
                selected_rows = data[data['player'] == selected_player]

                # Convert DataFrame to dictionary for serialization
                selected_rows_dict = selected_rows.to_dict('records')

                # Store the selected rows in session
                request.session['selected_rows'] = selected_rows
                request.session['columns'] = list(data.columns)

                return render_selection_result(request, selected_player)
    else:
        form = UploadCSVForm()
